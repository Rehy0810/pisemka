...
class LogUserForm(Form):
    ...
    prijmeni = TextField('Choose your password', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=6, max=30, message="Please use between 6 and 30 characters"),
        InputRequired(message="You can't leave this empty")
    ])
...
class GameUserForm(Form):
    ...
    username = TextField('Choose your password', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=10, max=30, message="Please use between 10 and 30 characters"),
        InputRequired(message="You can't leave this empty")
    ])
